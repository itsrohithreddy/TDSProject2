...
os.makedirs(sys.argv[1].split(".csv")[0],exist_ok=True)
...
content = (
    ...
    f"save the chart in {sys.argv[1].split(".csv")[0]} folder."
)
...
f"Save/Exort both the scatter plot and heatmap of the correlation matrix separately in two png files inside {sys.argv[1].split(".csv")[0]} folder."
)
...
image_path = sys.argv[1].split(".csv")[0]+"/"+CHART_BINNABLE
...
# visualization_data = []
# for file in os.listdir(sys.argv[1].split(".csv")[0]):
#   if file.endswith(".png"):
#     b64_image = encode_image(os.path.join(sys.argv[1].split(".csv")[0],file))
#     SYS_CONTENT = """
...
process_all_images(sys.argv[1].split(".csv")[0])
print("Images vis:",time.time() - start)



# # Create a ReadME.md file using the given data
markdown = f"# Analysis Report for `{sys.argv[1].split(".csv")[0]}`\n\n"
...
with open(f"{sys.argv[1].split(".csv")[0]}/README.md","w") as f:
    f.write(readME)
print(readME)
